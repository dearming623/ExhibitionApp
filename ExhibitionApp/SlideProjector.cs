...
class SlideProjector
{
    ...
    private void ClearBackground()
    {
        ...
        //重画原图
        RectangleF destRect = new RectangleF(0, 0, bmpWidth, bmpHeight);
        dc.DrawImage(new Bitmap(backgroundBmp, bmpWidth, bmpHeight), destRect);
    ...
    //public SlideProjector(Bitmap inBmp)
    //{
    //    delay = 1;
    ...
    public SlideProjector(int width, int height)
    {
        bmpWidth = width;
        bmpHeight = height;
    }
    ...
    public void loadImages(ArrayList lst)
    {
        images = lst;
    }
    ...
    public string randomImagePath()
    {
        if (images != null && images.Count > 0)
        {
            ...
            int idx = ran.Next(0, images.Count);

            return Convert.ToString( images[idx]);
        }
    ...
    public void workThread()
    {
        ...
        while (!stop)
        {
            ...
            this.bmp =  new Bitmap(img, bmpWidth, bmpHeight);

            outBmp = new Bitmap(bmpWidth, bmpHeight);
            ...
            backgroundBmp =(Bitmap) OutBmp.Clone();
        }
    ...
    public void Start()
    {
        ...
        drawThread = new Thread(workThread);
    ...
    private void doAnimator(int type)
    {
    ...
    private void Animator03()
    {
        ...
        try
        {
            ...
            for (float x = 0; x <= bmp.Width / 2f; x += stepCount)
            {
                ...
                dc.DrawImage(backgroundBmp, rect);
            ...
            backgroundBmp = bmp.Clone() as Bitmap;
        }
        catch (Exception ex)
        {
            ShowError(ex.Message);
