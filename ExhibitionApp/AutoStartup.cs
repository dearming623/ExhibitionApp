...
namespace ExhibitionApp
{
    class AutoStartup
    {
        static string Key = "Exhibition_" + Application.StartupPath.GetHashCode();
            ...
            try
            {
                ...
                foreach (string item in runList)
                {
                    if (item.Equals(Key, StringComparison.OrdinalIgnoreCase))
                        return true;
                    else if (item.Equals("Exhibition", StringComparison.OrdinalIgnoreCase)) // Compatibility with older versions
                    {
